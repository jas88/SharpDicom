...
namespace Dicom
{
    ...
    /// Matches fo-dicom 4.x DicomDataset API surface with Get&lt;T&gt; as primary accessor.
    /// </summary>
    public sealed class DicomDataset : IEnumerable<DicomItem>
    {
        ...
        /// This is the primary fo-dicom 4.x API (dataset.Get&lt;T&gt;(tag)).
        /// </summary>
        /// <typeparam name="T">The target type (string, int, double, DicomUID).</typeparam>
        /// <param name="tag">The tag to retrieve.</param>
        /// <param name="index">The value index (0-based, for multi-valued elements).</param>
        /// <returns>The value.</returns>
#pragma warning disable CA1716 // Must match fo-dicom 4.x API name exactly
        public T Get<T>(DicomTag tag, int index = 0)
#pragma warning restore CA1716
        {
            return GetValueInternal<T>(tag, index);
        }

        /// <summary>
        /// Gets a single typed value for the specified tag, returning a default value if missing.
        /// This is the fo-dicom 4.x API overload: dataset.Get&lt;T&gt;(tag, defaultValue).
        /// </summary>
        /// <typeparam name="T">The target type (string, int, double, DicomUID).</typeparam>
        /// <param name="tag">The tag to retrieve.</param>
        /// <param name="defaultValue">The default value if the tag is not present.</param>
        /// <returns>The value, or the default if the tag is missing.</returns>
#pragma warning disable CA1716 // Must match fo-dicom 4.x API name exactly
        public T Get<T>(DicomTag tag, T defaultValue)
#pragma warning restore CA1716
        {
            if (!_inner.Contains(tag.ToSharpDicom()))
                return defaultValue;

            try
            {
                return GetValueInternal<T>(tag, 0);
            }
            catch
            {
                return defaultValue;
        ...
        /// <summary>
        /// Gets a single typed value for the specified tag.
        /// Also available in late fo-dicom 4.x as an alias for Get&lt;T&gt;.
        /// </summary>
        /// <typeparam name="T">The target type (string, int, double, DicomUID).</typeparam>
        /// <param name="tag">The tag to retrieve.</param>
        /// <returns>The value.</returns>
        ...
        /// <summary>
        /// Internal shared implementation for retrieving typed values.
        /// </summary>
        private T GetValueInternal<T>(DicomTag tag, int index)
        {
            var sdTag = tag.ToSharpDicom();
