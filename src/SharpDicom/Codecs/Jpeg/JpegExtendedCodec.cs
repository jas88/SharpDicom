...
{
    /// <summary>
    /// JPEG Extended (Process 2,4) codec implementing IPixelDataCodec.
    /// </summary>
    /// <remarks>
    /// <para>
    /// This codec implements DICOM Transfer Syntax 1.2.840.10008.1.2.4.51 (JPEG Extended).
    /// It provides lossy compression for 8-bit and 12-bit grayscale and RGB images using
    /// the Extended Sequential DCT process (SOF1).
    /// </para>
    /// <para>
    /// JPEG Extended (Process 2,4) is commonly used in DICOM for medical images that
    /// require 12-bit sample precision, such as CT and CR modalities. Process 2 is
    /// the baseline process for 12-bit and Process 4 is extended sequential for 12-bit.
    /// </para>
    /// <para>
    /// Key characteristics:
    /// <list type="bullet">
    /// <item>Lossy compression using DCT</item>
    /// <item>Supports 8-bit and 12-bit sample precision</item>
    /// <item>SOF1 marker (Extended Sequential DCT)</item>
    /// <item>Huffman coding for entropy compression</item>
    /// </list>
    /// </para>
    /// </remarks>
    public sealed class JpegExtendedCodec : IPixelDataCodec
    {
        ...
        /// <inheritdoc />
        public string Name => "JPEG Extended (Process 2,4) - 8/12-bit";
            ...
            Memory<byte> destination)
        {
            ...
            return JpegExtendedDecoder.DecodeFrame(fragment.Span, info, destination.Span, frameIndex);
        }
            ...
            object? options = null)
        {
            ...
            for (int i = 0; i < frameCount; i++)
            {
                ...
                var encoded = JpegExtendedEncoder.EncodeFrame(frameData, info, jpegOptions);
        ...
        public ValidationResult ValidateCompressedData(DicomFragmentSequence fragments, PixelDataInfo info)
        {
            ...
            for (int i = 0; i < fragments.Fragments.Count; i++)
            {
                ...
                // Find and validate SOF1 marker (or SOF0 for 8-bit fallback)
                int position = 2;
                bool foundSof = false;
                ...
                while (position < span.Length - 1 && !foundSof)
                {
                    ...
                    // Check for SOF1 (primary) or SOF0 (acceptable for 8-bit)
                    if (marker == JpegMarkers.SOF1 || marker == JpegMarkers.SOF0)
                    {
                        foundSof = true;
                        ...
                        if (JpegFrameInfo.TryParse(payload, marker, out var frameInfo))
                        {
                            ...
                            // Validate precision is 8 or 12
                            if (frameInfo.Precision != 8 && frameInfo.Precision != 12)
                            {
                                issues.Add(new CodecDiagnostic(
                                    i, position,
                                    "Unsupported precision for JPEG Extended",
                                    "8 or 12",
                                    frameInfo.Precision.ToString(System.Globalization.CultureInfo.InvariantCulture)));
                            }
                        }
                    }
                    else if (JpegMarkers.IsSOF(marker))
                    {
                        issues.Add(new CodecDiagnostic(
                            i, position - 2,
                            "Unsupported SOF marker for JPEG Extended",
                            "0xC1 (SOF1) or 0xC0 (SOF0)",
                            $"0x{marker:X2}"));
                    }
                ...
                if (!foundSof)
                {
                    issues.Add(new CodecDiagnostic(i, 0, "No SOF1 or SOF0 marker found", null, null));
                }
            }
