...
{
    /// <summary>
    /// JPEG 2000 Lossy codec implementing IPixelDataCodec.
    /// </summary>
    /// <remarks>
    /// <para>
    /// This codec implements DICOM Transfer Syntax 1.2.840.10008.1.2.4.91
    /// (JPEG 2000 Image Compression).
    /// </para>
    /// <para>
    /// It uses the irreversible 9/7 wavelet transform and irreversible color transform (ICT)
    /// as specified in ITU-T T.800 (JPEG 2000 Part 1). This provides excellent compression
    /// with controlled quality loss.
    /// </para>
    /// <para>
    /// Key characteristics:
    /// <list type="bullet">
    /// <item>Lossy compression - quality loss based on compression ratio</item>
    /// <item>Supports 2-16 bit samples</item>
    /// <item>Supports grayscale and RGB images</item>
    /// <item>Typical compression ratio: 10:1 to 50:1 depending on quality requirements</item>
    /// </list>
    /// </para>
    /// </remarks>
    public sealed class Jpeg2000LossyCodec : IPixelDataCodec
    {
        ...
        public CodecCapabilities Capabilities { get; } = new(
            CanEncode: true,
            CanDecode: true,
            IsLossy: true,  // Key difference from lossless
        ...
        public DicomFragmentSequence Encode(
            ...
            object? options = null)
        {
            var j2kOptions = options is Jpeg2000CodecOptions opt ? opt : Jpeg2000CodecOptions.MedicalImaging;
        ...
        public ValidationResult ValidateCompressedData(DicomFragmentSequence fragments, PixelDataInfo info)
        {
            ...
            for (int i = 0; i < fragments.Fragments.Count; i++)
            {
                ...
                // Lossy codec can accept both lossy and lossless codestreams
                // (decoding a lossless codestream as lossy is fine)
            }
