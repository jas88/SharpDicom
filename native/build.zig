...
pub fn build(b: *std.Build) void {
    ...
    for (targets) |target_query| {
        ...
        // x265 (HEVC encoding) is skipped for macOS due to C++ cross-compilation issues
        const has_x265 = target_query.os_tag != .macos;
        addFfmpegEncSources(lib, b, has_x265);
        ...
        // Skip x265 for macOS due to Zig libc++ cross-compilation issues with C++ threading
        // (timespec/nanosleep not properly exposed to libc++ headers in Zig's macOS sysroot)
        if (target_query.os_tag != .macos) {
    ...
    // Native builds always have x265 support (no cross-compilation issues)
    addFfmpegEncSources(native_lib, b, true);
...
fn addFfmpegEncSources(lib: *std.Build.Step.Compile, b: *std.Build, has_x265: bool) void {
    ...
    // Note: -O2 required before -D_FORTIFY_SOURCE=2 for glibc compatibility
    const ffmpeg_flags_base = &[_][]const u8{
        "-std=c11",
        "-O2", // Required for _FORTIFY_SOURCE with glibc
        "-fstack-protector-strong",
        "-D_FORTIFY_SOURCE=2",
    ...
    // Add x265 define only when x265 is available (not on macOS due to C++ threading issues)
    const ffmpeg_flags = if (has_x265)
        ffmpeg_flags_base ++ &[_][]const u8{"-DSHARPDICOM_HAS_X265"}
    else
        ffmpeg_flags_base;
    ...
    for (all_source_groups) |sources| {
        for (sources) |src| {
            // Skip libx265.c when x265 is not available (macOS due to C++ threading issues)
            if (!has_x265 and std.mem.eql(u8, src, "libavcodec/libx265.c")) {
                continue;
            }
