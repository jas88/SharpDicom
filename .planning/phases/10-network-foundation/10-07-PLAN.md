...
private byte[] SerializeCommand(DicomCommand command)
{
    ...
    var writer = new DicomStreamWriter(buffer, false, true); // Implicit VR LE for command (per PS3.7 Section 6.3.1)
...
{
    var reader = new DicomStreamReader(data.Span, false, true); // Implicit VR LE for command
