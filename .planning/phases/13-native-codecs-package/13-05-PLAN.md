...
4. Update `native/build.zig`:
  ...
  <verify>
```bash
# Verify GPU availability check and dynamic loading
...
# Verify CPU fallback to j2k_decode() is implemented
grep "j2k_decode" /Users/jas88/Developer/Github/SharpDicom/native/src/gpu_wrapper.c
```
GPU wrapper implements availability check, dynamic loading, AND CPU fallback to j2k_decode()
  </verify>
  <done>
GPU dispatch wrapper provides runtime detection of NVIDIA GPU and verified fallback to CPU via j2k_decode()
  </done>
</task>
