...
2. Call DicomClient.SendDimseRequestAsync(pcid, command, dataset, ct) - uses negotiated TS for dataset
3. Call DicomClient.ReceiveDimseResponseAsync(ct) to get C-STORE-RSP
4. Report progress at PDU boundaries
5. Handle timeout via CancellationTokenSource.CancelAfter
6. Support retry with exponential backoff on retryable failures (0xA7xx Out of Resources)
...
Uses DicomClient internal DIMSE primitives from Plan 11-01 Task 4:
- SendDimseRequestAsync(pcid, command, dataset, ct): Sends command + optional dataset with correct encoding
- ReceiveDimseResponseAsync(ct): Receives response command + optional dataset

Note: SendDimseRequestAsync handles:
- Command PDV encoding as Implicit VR LE
- Dataset encoding using negotiated transfer syntax
- PDV fragmentation respecting MaxPduLength
- Last fragment flag (0x02) on final PDV
  </action>
  <verify>
`dotnet build src/SharpDicom/SharpDicom.csproj` compiles without errors
  </verify>
