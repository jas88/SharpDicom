...
namespace SharpDicom.Network.Dimse.Services
{
    ...
    public sealed class CGetScu
    {
        ...
        private async Task SendCGetRequestAsync(DicomCommand command, DicomDataset identifier, CancellationToken ct)
        {
            // Get presentation context ID for C-GET SOP Class
        ...
        private async Task<(DicomCommand command, DicomDataset? dataset)> ReceiveMessageAsync(CancellationToken ct)
        {
            // Receive message using DicomClient DIMSE primitives (from 11-01 Task 4)
            // May be C-STORE-RQ (from SCP) or C-GET-RSP
            return await _client.ReceiveDimseResponseAsync(ct).ConfigureAwait(false);
        }

        private async Task SendCStoreResponseAsync(ushort messageId, DicomUID sopClassUid, DicomUID sopInstanceUid, DicomStatus status, CancellationToken ct)
        {
            // Build C-STORE-RSP
            var response = DicomCommand.CreateCStoreResponse(messageId, sopClassUid, sopInstanceUid, status);
            var pcid = GetPresentationContextId(sopClassUid);
            // Send command-only (no dataset for response)
            await _client.SendDimseRequestAsync(pcid, response, null, ct).ConfigureAwait(false);
        }

        private async Task SendCCancelAsync(ushort messageId, CancellationToken ct)
        {
            await _client.SendCCancelAsync(messageId, ct).ConfigureAwait(false);
        }
    }
}
```
