...
namespace SharpDicom.Tests.Codecs
{
    ...
    public class JpegBaselineCodecTests
    {
        ...
        [Test]
        public void EncodeAndDecode_ExtremeValues_Succeeds()
        {
        ...
        [Test]
        public void EncodeAndDecode_AllZeros_Succeeds()
        {
            ...
            // For uniform images, decoded values should be very close to original
            int maxDiff = 0;
            for (int i = 0; i < 256; i++)
            {
                maxDiff = Math.Max(maxDiff, Math.Abs(decoded[i] - original[i]));
            }
            Assert.That(maxDiff, Is.LessThan(10), $"Uniform black should decode close to original (max diff: {maxDiff})");
        }

        [Test]
        public void EncodeAndDecode_AllMax_Succeeds()
        {
            ...
            Assert.That(maxDiff, Is.LessThan(10), $"Uniform white should decode close to original (max diff: {maxDiff})");
        }

        [Test]
        public void EncodeAndDecode_SingleMCU_Succeeds()
        {
            // Minimum JPEG image: 8x8 (one MCU)
            var info = PixelDataInfo.Grayscale8(8, 8);
            var original = new byte[64];
            ...
            Assert.That(result.Success, Is.True, "Single MCU should encode/decode successfully");
        }

        [Test]
        public void EncodeAndDecode_LargerImage_Succeeds()
        {
        ...
        [Test]
        public void EncodeAndDecode_NonMultipleOf8Width_Succeeds()
        {
            // Width not multiple of 8 (requires MCU padding)
            var info = PixelDataInfo.Grayscale8(15, 16);
            var original = new byte[15 * 16];

            var random = new Random(42);
            random.NextBytes(original);

            var fragments = _codec.Encode(original, info);
            var decoded = new byte[15 * 16];
            var result = _codec.Decode(fragments, info, 0, decoded);

            Assert.That(result.Success, Is.True, "Non-multiple-of-8 width should work");
        }

        [Test]
        public void EncodeAndDecode_NonMultipleOf8Height_Succeeds()
        {
            // Height not multiple of 8 (requires MCU padding)
            var info = PixelDataInfo.Grayscale8(16, 13);
            var original = new byte[16 * 13];

            var random = new Random(42);
            random.NextBytes(original);

            var fragments = _codec.Encode(original, info);
            var decoded = new byte[16 * 13];
            var result = _codec.Decode(fragments, info, 0, decoded);

            Assert.That(result.Success, Is.True, "Non-multiple-of-8 height should work");
        }
